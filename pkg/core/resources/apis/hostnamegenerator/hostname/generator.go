...
type HostnameGenerator interface {
	...
	GenerateHostname(localZone string, generator *hostnamegenerator_api.HostnameGeneratorResource, resource model.Resource, validateDNS bool) (string, error)
}
	...
	for _, generatorType := range g.generators {
		...
		for _, generator := range sortGenerators(generators.Items) {
			for _, service := range resources {
				...
				generated, err := generatorType.GenerateHostname(g.zone, generator, service, true)
...
func EvaluateTemplate(localZone string, generatorTemplate string, meta model.ResourceMeta, validateDNS bool) (string, error) {
	sb := strings.Builder{}
	...
	generated := sb.String()
	// The read-only inspect path passes validateDNS=false so that hostnames
	// generated by older zones (before this RFC 1123 check existed) still render
	// instead of failing the whole request. Generation itself keeps validateDNS=true.
	if validateDNS {
