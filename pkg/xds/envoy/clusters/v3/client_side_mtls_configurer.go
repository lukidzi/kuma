...
func (c *ClientSideMTLSConfigurer) createTransportSocket(sni string) (*envoy_core.TransportSocket, error) {
	...
	tlsContext, err := envoy_tls.NewCreateUpstreamTlsContext(identity, ca, c.UpstreamService, sni, verifyIdentities, c.Namespace, c.SA, c.TrustedDomain)
	if err != nil {
		return nil, err
	}
	if tlsContext == nil {
