...
func (e *MeshEnv) WithZone(cluster Cluster, installs ...InstallFunc) *MeshEnv {
	e.zones = append(e.zones, meshEnvZone{cluster: cluster, installs: installs})
	return e
}
...
func (e *MeshEnv) Setup() error {
	...
	if err := WaitForMesh(e.Mesh, e.clusters()); err != nil {
		return err
	}
	...
	for _, zone := range e.zones {
		...
		setup.Install(Parallel(e.zoneInstalls(zone)...)).SetupInGroup(zone.cluster, &group)
	}
